...
export function StoreOrders() {
  ...
  const getFilteredOrders = () => {
    ...
    if (statusFilter !== 'all') {
      const statusMap: Record<StatusFilter, string[]> = {
        all: [],
        pending: ['PENDING_PAYMENT', 'PAID', 'pending', 'paid'],
  ...
  const statusCounts = {
    all: orders.length,
    pending: orders.filter(o => ['PENDING_PAYMENT', 'PAID', 'pending', 'paid'].includes(o.status)).length,
    accepted: orders.filter(o => ['ACCEPTED', 'accepted'].includes(o.status)).length,
    shipped: orders.filter(o => ['SHIPPED', 'DELIVERED', 'shipped', 'delivered'].includes(o.status)).length,
    completed: orders.filter(o => ['COMPLETED', 'completed'].includes(o.status)).length,
    disputed: orders.filter(o => ['DISPUTED', 'CANCELLED', 'disputed', 'cancelled'].includes(o.status)).length,
  };
      ...
      {filteredOrders.length === 0 ? (
        ...
        <div className="space-y-3">
          {filteredOrders.map((order) => {
            ...
            const needsAction = ['PAID', 'paid', 'pending'].includes(order.status);
