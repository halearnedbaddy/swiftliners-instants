    ...
    if (ResultCode === 0) {
      ...
      if (transaction.seller?.phone) {
        await sendSMS(
          ...
          `PayLoom: KES ${paidAmount || transaction.amount} payment secured for "${transaction.itemName}". Ship the item now!`
        );
      }
...
router.post('/confirm-delivery', authenticate, async (req, res) => {
  try {
    ...
    if (transaction.seller?.phone) {
      await sendSMS(
        transaction.seller.phone,
        `PayLoom: KES ${sellerPayout.toLocaleString()} has been released to your wallet for "${transaction.itemName}". Withdraw anytime!`
      );
    }
