...
async function payoutToSeller(
  ...
  try {
    ...
    const payload = {
      ...
      QueueTimeOutURL: `${apiUrl}/functions/v1/payment-api/webhook/timeout`,
      ResultURL: `${apiUrl}/functions/v1/payment-api/webhook/b2c-result`,
  ...
  try {
    ...
    if (action === "verify" && req.method === "POST") {
      ...
      try {
        ...
        const initiatorName = Deno.env.get("MPESA_INITIATOR_NAME");
        const securityCredential = Deno.env.get("MPESA_SECURITY_CREDENTIAL");
        const resultUrl = Deno.env.get("MPESA_RESULT_URL") || `${supabaseUrl}/functions/v1/payment-api/webhook/verification-result`;
        const timeoutUrl = Deno.env.get("MPESA_QUEUE_TIMEOUT_URL") || `${supabaseUrl}/functions/v1/payment-api/webhook/timeout`;
    ...
    if (action === "account-balance" && req.method === "POST") {
      ...
      try {
        ...
        const token = await getAccessToken();
        const resultUrl = `${supabaseUrl}/functions/v1/payment-api/webhook/balance-result`;
        const timeoutUrl = `${supabaseUrl}/functions/v1/payment-api/webhook/timeout`;
    ...
    if (action === "register-c2b" && req.method === "POST") {
      ...
      try {
        ...
        const token = await getAccessToken();
        const confirmationUrl = `${supabaseUrl}/functions/v1/payment-api/webhook/c2b-confirmation`;
        const validationUrl = `${supabaseUrl}/functions/v1/payment-api/webhook/c2b-validation`;
